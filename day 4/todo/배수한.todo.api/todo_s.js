...
function btnClickEvent(event) {
  event.preventDefault(); //form 안에 있는 submit버튼이라서 해줘야함
...
let myMap = new Map(); //전역 변수로 선언된 map
const userId = "sh";
...
/** getToServerMonth
 * 연-월로 request를 보낸 뒤 결과값을 받아 handleSameKey() 를 사용해 map에 저장
 */

function getToServerMonth() {
  myMap.clear();
  let arr = [`${nowYear}`, `${nowMonth}`];

  const targetDate = `${arr[0]}-${arr[1].length == 1 ? "0" + arr[1] : arr[1]}`;
  const promise = new Promise(function (resolve, reject) {
    const xhr = new XMLHttpRequest();
    ...
    for (let idx in result) {
      let obj = {
        date: result[idx].eventDt,
        cat: "개인",
        todo: result[idx].subject,
      };
      handleSameKey(obj);
...
/** handleSameKey
 *  동일한 키 값 여부에 따라 value를 로컬스토리지에 삽입
 */
function handleSameKey(info) {
  if (myMap.has(info.date)) {
    let arrVal = myMap.get(info.date);
    arrVal.push(info);
    myMap.set(info.date, arrVal);
  } else {
    const arrObj = [info];
    myMap.set(info.date, arrObj);
  }
}
